...
It runs only on the routes listed in `DO_BASE_BANNER_ROUTES` and on a preview link, which are the pages that draw a banner. The edit form, the delete confirmation and the revision list all carry a node parameter and resolve the same background without ever rendering it, and a preload the page does not use is a wasted request for a full-width derivative.
