...
- **Pages already in the URL are put back before a list runs.** A pager link names a page for every element the pager manager knows about when the link is built, and each list registers its element only when its own turn comes. Without restoring them first, the list that runs first would emit links naming only its own element and send every later list back to page 1.
