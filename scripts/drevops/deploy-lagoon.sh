...
# ACTION: 'deploy' OR 'deploy_override_db'
  ...
  if [ -n "${DREVOPS_DEPLOY_PR:-}" ]; then
    ...
    if [ "${is_redeploy:-}" = "1" ]; then
      ...
      lagoon deploy pullrequest -n "${DREVOPS_DEPLOY_PR}" --base-branch-name "${DREVOPS_DEPLOY_PR_BASE_BRANCH}" -R "origin/${DREVOPS_DEPLOY_PR_BASE_BRANCH}" -H "${DREVOPS_DEPLOY_BRANCH}" -M "${DREVOPS_DEPLOY_PR_HEAD}" -t "${deploy_pr_full}"
