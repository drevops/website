...
trait RobotsTrait {
  ...
  public function pathsAllowedForRobots(TableNode $table): void {
    ...
    foreach ($rows as $row) {
      ...
      $disallowPattern = 'Disallow: ' . preg_quote($path, '/');
      if (preg_match('/' . $disallowPattern . '/', $robotsTxt)) {
      ...
      for ($i = 0; $i < count($pathParts); $i++) {
        ...
        if (preg_match('/' . $partialDisallowPattern . '/', $robotsTxt)) {
  ...
  public function pathsDisallowedForRobots(TableNode $table): void {
    ...
    foreach ($rows as $row) {
      ...
      // Check exact match.
      $exactPattern = 'Disallow: ' . preg_quote($path, '/');
      if (preg_match('/' . $exactPattern . '/', $robotsTxt)) {
        $disallowFound = TRUE;
        continue;
      }
