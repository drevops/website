...
Feature: ClamAV Anti-virus
  ...
  Scenario: Upload EICAR test file to trigger virus detection.
    ...
    And I go to "media/add/civictheme_document"
    When I attach the file "public://eicar_test.txt" to "files[field_c_m_document_0]"
    And press "Upload"
  ...
  Scenario: Upload test file to ensure that file upload works
    Given I am logged in as a user with the "administrator" role
    And I go to "media/add/civictheme_document"
    When I attach the file "public://test.txt" to "files[field_c_m_document_0]"
    And press "Upload"
